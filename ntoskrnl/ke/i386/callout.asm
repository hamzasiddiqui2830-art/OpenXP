...
Kcb10:  push    dword ptr [ebx+ThCallbackStack]   ; save callback stack address
        mov     edx,dword ptr [ebx+ThTrapFrame]   ; get current trap frame address
        push    edx                     ; save trap frame address
        mov     esi,dword ptr [ebx+ThInitialStack] ; get initial stack address
        push    esi                     ; save initial stack address
        mov     dword ptr [ebx+ThCallbackStack],esp ; save callback stack address
...
;
        ...
        mov     ecx,dword ptr [esi+FpControlWord] ; copy NPX state to new frame
        mov     dword ptr [esp+FpControlWord],ecx ;
        mov     ecx,dword ptr [esi+FpStatusWord]  ;
        mov     dword ptr [esp+FpStatusWord],ecx  ;
        mov     ecx,dword ptr [esi+FpTagWord]     ;
        mov     dword ptr [esp+FpTagWord],ecx     ;
        mov     ecx,dword ptr [esi+FxMXCsr]       ;
        mov     dword ptr [esp+FxMXCsr],ecx       ;
        mov     ecx,dword ptr [esi+FpCr0NpxState] ;
        mov     dword ptr [esp+FpCr0NpxState],ecx ;
        mov     esi,PCR[PcTss]          ; get address of task switch segment
        mov     dword ptr [ebx+ThInitialStack],edi ; reset initial stack address
        mov     ecx, esp                ;
...
@@:     mov     dword ptr [esi+TssEsp0],ecx       ; set kernel entry stack address
...
;
        ...
        mov     edi, dword ptr [edx+TsDr7]                ; Fetch control register
        test    edi, (NOT DR7_RESERVED_MASK)    ; Do we need to restore Debug reg?
        mov     dword ptr [esp+TsDr7], edi                ; Save away Dr7
        jnz     short Kcb18                     ; Yes, go save them.

Kcb15:  mov     eax,_KeUserCallbackDispatcher ; st address of callback dispatcher
        mov     dword ptr [esp+TsEip],eax         ;
        mov     eax,PCR[PcExceptionList] ; get current exception list
        mov     dword ptr [esp+TsExceptionList],eax ; set previous exception list
        mov     eax,dword ptr [edx+TsPreviousPreviousMode] ; get previous mode
        mov     dword ptr [esp+TsPreviousPreviousMode],eax ; set previous mode
...
;
        ...
        add     ebp,edi                 ;
        mov     eax,dword ptr [edx+ThTrapFrame]   ; relocate the current trap frame address
        sub     eax,ecx                 ;
        add     eax,edi                 ;
        mov     dword ptr [edx+ThTrapFrame],eax   ;
...
;
        ...
        mov     byte ptr dword ptr [edx+ThLargeStack], 1 ; set large stack TRUE
        mov     dword ptr [edx+ThInitialStack],ecx ; set new initial stack address
...
@@:     mov     dword ptr [edx+TssEsp0],ecx       ; set kernel entry stack address
...
cPublicProc _KiGetUserModeStackAddress, 0
        ...
        mov     eax,dword ptr [eax+ThTrapFrame]   ; get current trap frame address
        lea     eax,dword ptr [eax+TsHardwareEsp] ; get address of stack address
...
cPublicProc _NtCallbackReturn, 3
        ...
        mov     eax,PCR[PcPrcbData + PbCurrentThread] ; get current thread address
        mov     ecx,dword ptr [eax+ThCallbackStack] ; get callback stack address
        test    ecx, ecx
...
;
        ...
        mov     ebx, dword ptr [eax+ThTrapFrame]   ; get current trap frame address
        mov     edx, dword ptr [ebx+TsExceptionList] ; get saved exception list address
        ...
        mov     ebx, dword ptr [ecx+CuOutBf]       ; get address to store output buffer
        mov     [ebx], edi               ; store output buffer address
        mov     ebx, dword ptr [ecx+CuOutLn]       ; get address to store output length
        mov     [ebx], esi               ; store output buffer length
        mov     ebx, [ecx]               ; get previous initial stack address
        cli                             ; disable interrupt
        mov     esi, dword ptr [eax+ThInitialStack] ; get source NPX save area address
        mov     dword ptr [eax+ThInitialStack], ebx ; restore initial stack address
        sub     esi, NPX_FRAME_LENGTH    ; compute source NPX save area
        sub     ebx, NPX_FRAME_LENGTH    ; compute destination NPX save area
        mov     edx,dword ptr [esi+FpControlWord] ; copy NPX state to previous frame
        mov     dword ptr [ebx+FpControlWord], edx ;
        mov     edx, dword ptr [esi+FpStatusWord]  ;
        mov     dword ptr [ebx+FpStatusWord], edx  ;
        mov     edx, dword ptr [esi+FpTagWord]     ;
        mov     dword ptr [ebx+FpTagWord], edx     ;
        mov     edx, dword ptr [esi+FxMXCsr]       ;
        mov     dword ptr [ebx+FxMXCsr], edx       ;
        mov     edx, dword ptr [esi+FpCr0NpxState] ;
        mov     dword ptr [ebx+FpCr0NpxState], edx ;
        cmp     ebp, STATUS_CALLBACK_POP_STACK
        mov     edi, dword ptr [ecx+CuTrFr]        ; Fetch address of previous trap frame
        jz      short _NtCbCopyTrapFrame
_NtCbCopyTrapFrameRet:
        and     dword ptr dword ptr [edi+TsDr7], 0 ; Assume original trap frame has no debug registers active
        test    byte ptr dword ptr [eax+ThDebugActive], 0ffh 
...
@@:
        mov     dword ptr [edx+TssEsp0], ebx       ; restore kernel entry stack address
        mov     dword ptr [eax+ThTrapFrame], edi  ; restore current trap frame address
        sti                             ; enable interrupts
        pop     dword ptr [eax+ThCallbackStack]   ; restore callback stack address
...
@@:     lea     esi, dword ptr [esi+TsSegFs]
        rep     movsd                    ; copy the relevant parts of the frame
        mov     ecx, dword ptr [eax+ThCallbackStack]
...
;
        mov     esi, dword ptr [eax+ThTrapFrame]  ; Get current trap frame
        mov     edx, dword ptr [esi+TsDr0]
        mov     dword ptr [edi+TsDr0], edx
        mov     edx, dword ptr [esi+TsDr1]
        mov     dword ptr [edi+TsDr1], edx
        mov     edx, dword ptr [esi+TsDr2]
        mov     dword ptr [edi+TsDr2], edx
        mov     edx, dword ptr [esi+TsDr3]
        mov     dword ptr [edi+TsDr3], edx
        mov     edx, dword ptr [esi+TsDr6]
        mov     dword ptr [edi+TsDr6], edx
        mov     edx, dword ptr [esi+TsDr7]
        mov     dword ptr [edi+TsDr7], edx
