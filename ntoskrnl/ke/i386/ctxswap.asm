...
;
         ...
         push    dword ptr [ebx+PcExceptionList]
         mov     dword ptr [ebx+PcExceptionList], EXCEPTION_CHAIN_END
...
;
         ...
         pop     esp
         pop     dword ptr [ebx+PcExceptionList]
         pop     ebp
...
sc01:   mov     eax, dword ptr [ebx+PcContextSwitches] ; get current count
         inc     eax                     ; increment it
         mov     dword ptr [ebx+PcContextSwitches], eax ; store it back

;
; Save the thread exception list head.
;

         push    dword ptr [ebx+PcExceptionList] ; save thread exception list head

;
; Check for context swap logging.
;

         cmp     dword ptr [ebx+PcPerfGlobalGroupMask], 0 ; check if logging enable
...
;
         ...
         pop     dword ptr [ebx+PcExceptionList] ; restore thread exception list head
...
;
; Check if new thread selected
;

CheckNextThread:
         mov     eax, [ebx+PcPrcbData+PbNextThread] ; check if new thread selected
         cmp     eax, 0                  ; NULL?
         je      short CheckDpcList0

IF DBG

         xor     edi, edi                ; clear poll counter
...
@KiIdleLoop@0 endp
         ...
         PAGE ,132
         SUBTTL  "I386 specific, nonpaged code"

         align   4

; _Ki386CheckDelayedNpxTrap - See ki386.c for description.
;

         PUBLIC  _Ki386CheckDelayedNpxTrap

_Ki386CheckDelayedNpxTrap:

         mov     eax, cr0

         ; if TS is not set, there is no delayed FP trap

         test    eax, CR0_TS
         jz      short @f

         ; TS is set. Check if there's an active FP state for this thread
         ...
         ; There's active FP state. Raise an exception
