...
NTSTATUS RtlpSecMemFreeVirtualMemory(IN HANDLE ProcessHandle, IN OUT PVOID *BaseAddress, IN OUT PSIZE_T RegionSize, IN ULONG FreeType);
...
/* The WRK source expects this private activation-context helper. The current
 * tree does not provide the user-mode loader implementation, so report the
 * documented unavailable-query status instead of relying on an implicit MSVC
 * declaration (C4013). */
#ifndef RtlpThreadPoolGetActiveActivationContext
static __inline NTSTATUS
RtlpThreadPoolGetActiveActivationContext(
    OUT PACTIVATION_CONTEXT_HANDLE ActivationContext
    )
{
    if (ActivationContext != NULL) {
        *ActivationContext = INVALID_ACTIVATION_CONTEXT;
    }
